...
#define PINTHR_MAGIC 0xcd6abf4b
...
void thread_init(void)
{
  ...
  init_thread(initial_thread, "main", PRTY_DEFAULT);
  initial_thread->status = PINTHR_RUNNING;
...
void thread_start(void)
{
  ...
  sem_init(&idle_started, 0);
  thread_create("idle", PRTY_MIN, idle, &idle_started);

  /* Start preemptive thread scheduling. */
  intr_enable();

  /* Wait for the idle thread to initialize idle_thread. */
  sem_down(&idle_started);
}
...
thread_current(void)
{
  ...
  ASSERT(is_thread(t));
  ASSERT(t->status == PINTHR_RUNNING);
...
void thread_exit(void)
{
  ...
  thread_current()->status = PINTHR_DYING;
  schedule();
  NOT_REACHED();
}
...
{
  return t != NULL && t->magic == PINTHR_MAGIC;
}
...
void thread_schedule_tail(struct thread *prev)
{
  ...
  /* If the thread we switched from is dying, destroy its struct
     ...
     palloc().) */
  if (prev != NULL && prev->status == PINTHR_DYING && prev != initial_thread)
  {
    ASSERT(prev != cur);
